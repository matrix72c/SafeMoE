...
def _load_checkpoint_config(ckpt_dir: Path) -> Config:
    ...
    return Config(**{k: v for k, v in raw.items() if not isinstance(v, dict)})
...
def _load_model(ckpt_dir: Path, fabric: Optional[L.Fabric] = None) -> tuple:
    ...
    Returns (model: GPT, config: Config).
...
def _get_val_loaders(ckpt_dir: Path, config: Config, data_mock=None) -> dict:
...
def _ensure_surgery_checkpoint(ckpt_dir: Path) -> Path:
    ...
    *ckpt_dir* unchanged.  Surgery is idempotent: re-running it reuses the
    ...
    from litgpt.safemoe import surgery as _surgery_mod
